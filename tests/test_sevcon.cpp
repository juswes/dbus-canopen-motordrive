...
TEST_F(SevconTest, readSuccess) {
    ...
    EXPECT_EQ(nodes[0].device->motorTorque->variant.value.SN16, 80);
    EXPECT_EQ(nodes[0].device->controllerTemperature->variant.value.SN8, 30);
    EXPECT_EQ(nodes[0].device->motorDirection->variant.value.UN8, 2);
}

TEST_F(SevconTest, negativeMotorTorque) {
    VeRawCanMsg message;
    ...
    queue.push_back(
        {.canId = 0x581,
         .length = 8,
         .mdata = {0x42, 0x02, 0x46, 0x0C, 0xB0, 0xFF, 0x00, 0x00}});
    // Controller Temperature
    queue.push_back(
        {.canId = 0x581,
         .length = 8,
